            ...
            @if($post->image && !$isVideo)
                <div class="bg-white rounded-lg shadow-sm overflow-hidden post-item">
                    <div class="p-4">
                        ...
                        <!-- Display Comments - Modified to show only last comment by default -->
                        @if(isset($post->comments) && count($post->comments) > 0)
                            <div class="comments-container mt-3 pt-3 border-t">
                                ...
                                <div class="comment flex mb-3">
                                    <img src="{{ asset('storage/' . $lastComment->user->profile_photo_path) }}"
                                         alt="User Profile"
                                         class="w-8 h-8 rounded-full mr-2">
                                    <div class="bg-gray-100 rounded-lg py-2 px-3 flex-grow">
                                        <div class="font-medium text-sm">{{ $lastComment->user->name }}</div>
                                        <p class="text-sm text-gray-700">{{ $lastComment->content }}</p>
                                        <div class="text-xs text-gray-500 mt-1">{{ $lastComment->created_at->diffForHumans() }}</div>
                                    </div>
                                    @auth
                                        @if(Auth::user()->role_id == 1)
                                            <form action="{{ route('comments.destroy', $lastComment->id) }}" method="POST" class="ml-2">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <!-- Show More Comments Button (if there are more than 1 comment) -->
                                @if($commentsCount > 1)
                                    <button
                                        class="show-more-comments-btn text-sm text-blue-500 hover:text-blue-600 font-medium mb-3"
                                        data-post-id="{{ $post->id }}"
                                        onclick="toggleComments('{{ $post->id }}')"
                                    >
                                        Show {{ $commentsCount - 1 }} more {{ $commentsCount - 1 == 1 ? 'comment' : 'comments' }}
                                    </button>

                                    <!-- Hidden Comments Container -->
                                    <div id="hidden-comments-{{ $post->id }}" class="hidden-comments hidden">
...
<script>
    function toggleComments(postId) {
        const hiddenCommentsContainer = document.getElementById(`hidden-comments-${postId}`);
        const showMoreBtn = document.querySelector(`button[data-post-id="${postId}"]`);

        if (hiddenCommentsContainer.classList.contains('hidden')) {
            // Show comments
            hiddenCommentsContainer.classList.remove('hidden');
            showMoreBtn.textContent = 'Hide comments';
        } else {
            // Hide comments
            hiddenCommentsContainer.classList.add('hidden');
            const commentCount = hiddenCommentsContainer.querySelectorAll('.comment').length;
            showMoreBtn.textContent = `Show ${commentCount} more ${commentCount === 1 ? 'comment' : 'comments'}`;
        }
    }

    // Existing comment button functionality
